...
# --- Add symbols that exist only in scraper results ---
if os.path.exists(scraper_day_dir):
    for fname in os.listdir(scraper_day_dir):
        if not fname.endswith(".csv"):
            continue
        symbol = os.path.splitext(fname)[0]
        if symbol in seen_symbols:
            continue  # already processed via trade log

        scraper_file = os.path.join(scraper_day_dir, fname)
        with open(scraper_file, "r", encoding="utf-8-sig") as sf:
            reader = csv.DictReader(sf)
            scraper_data = next(reader, {})  # take first row

        # Empty trade data
        trade_data = {field: "" for field in trade_fields}
        trade_data["symbol"] = symbol  # at least keep the symbol
...
if rows:
    ...
    print("⚠️ No matching trade log or scraper entries found.")
