...
def log_stream_event(event_type, symbol, price, size, bid=None, ask=None, timestamp=None):
    """
    Додає подію стріму в буфер для подальшого запису у файл.
    """
    with log_lock:
        log_buffer.append((
...
async def background_csv_logger(filename="stream_log.csv", flush_interval=5):
    """
    Фонова задача: записує буфер подій у CSV раз на кілька секунд.
    """
    # Якщо файл ще не існує — створюємо з заголовком
    if not os.path.exists(filename):
        with open(filename, mode="w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["timestamp", "type", "symbol", "price", "size", "bid", "ask"])
...
# === Окремі обробники для логування стріму ===
async def log_quote(q):
    log_stream_event(
        "quote",
        q.symbol,
        q.ask_price,
...
async def main():
    ...
    for sym in symbols:
        ...
        stream.subscribe_quotes(log_quote, sym)
        stream.subscribe_trades(log_trade, sym)
    ...
    asyncio.create_task(background_csv_logger("stream_log.csv")) 
